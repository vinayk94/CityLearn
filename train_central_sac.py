...
from single_agent_sac import SAC_single_agent
import numpy as np
import csv
import time
import re
...
env = CityLearn(data_path, building_attributes, weather_file, solar_profile, building_id, buildings_states_actions = building_state_actions, cost_function = objective_function, verbose = 0,central_agent = True, simulation_period=(0,8760-1))
observations_spaces, actions_spaces = env.get_state_action_spaces()
central_agent_obs_space = env.observation_space
central_agent_act_space = env.action_space
...
agents = SAC_single_agent(building_id, building_state_actions, building_info, observation_spaces=central_agent_obs_space,
                 action_spaces=central_agent_act_space, hidden_dim=hid,start_training=8760*3, discount=gamma, tau=tau, lr=lr, batch_size=bs,
...
file_name = "central_agent_{}.csv".format(time.strftime("%Y-%m-%d %H:%M:%S"))
...
for e in range(n_episodes):
    ...
    with open("./central_agent.csv","a") as log:
