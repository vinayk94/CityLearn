...
parser.add_argument('--n_episodes', type=int, default=3, metavar='N',
...
agents = SAC_single_agent(building_id, building_state_actions, building_info, observation_spaces=observations_spaces,
                          action_spaces=actions_spaces, hidden_dim=hid, start_training=8760 * 1, discount=gamma,
                          tau=tau, lr=lr, batch_size=bs,
                          replay_buffer_capacity=1e6, exploration_period=8760 * 1 + 1,
...
for e in range(args.n_episodes):
    ...
    print('Episode',e,'Loss -', env.cost(), 'Simulation time (min) -', (time.time() - start) / 60.0, 'episode time(min) -',
