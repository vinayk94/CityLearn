...
class Ramping_Buffer:
    def __init__(self,size, dim):
        
        self.init_buffer = np.zeros((exploration_period-1,dim),dtype=np.float32)
        # note that the size refers to the no.of previous time steps stored.
        # dimensions refers to the number of features of the states that are stored.
        self.init_buffer_max_size=exploration_period-1
        self.init_buffer_dim = dim
        self.init_buffer_ptr =0
    ...
    def init_buffer_push(self,prev_dist_consumption):
        self.init_buffer[self.init_buffer_ptr] = last_step_sample
        self.init_buffer_ptr = (self.init_buffer_ptr+1) % self.init_buffer_max_size
        
    def find_init_ramping(self):
        last_day_cons = self.init_buffer[-24:]
        init_ramping = np.max(np.abs((last_day_cons - np.roll(last_day_cons,1))[1:]))
...
"""    
class Dist_Cons_Buffer:
    def __init__(self,size):
        self.buffer = np.zeros((size),dtype=np.float32)
        self.buffer[0] = 85.561
        self.ptr =1
        self.max_size=size
        #self.size = 0
    def push(self, last_step_sample):
...
class SAC_central_agent:
    def __init__(self,env, building_ids, buildings_states_actions, building_info, observation_spaces=None,
                 ...
                 replay_buffer_capacity=1e5, start_training = None,exploration_period = None,safe_exploration= False,
                        action_scaling_coef=1., reward_scaling=1., update_per_step=1,seed=0,pca_compression = 1. , add_previous_consumption=False, add_dist_cons= True,ramping_penalty = 0.0):
        ...
        #self.ramping_buffer = Ramping_Buffer(2, 1)
        #self.add_dist_cons = add_dist_cons
        #self.dist_cons_buffer = Dist_Cons_Buffer(2)
        self.ramping_penalty = ramping_penalty
        print(self.ramping_penalty)
        #self.action_list_ = []
        #self.action_list2_ = []
    ...
    def select_action(self, states, deterministic=False):
        ...
        if explore:
            ...
            #if self.add_dist_cons:
                #state_ = np.hstack((states, self.dist_cons_buffer.buffer[-1]))
    ...
    def add_to_buffer(self, states, actions, rewards, next_states, done):
        ...
        if self.env.net_electric_consumption[-1] < 0 and self.current_demand < 0 :
            rewards = rewards
        else:
            rewards = (1-self.ramping_penalty )*(rewards) - self.ramping_penalty *((np.abs(self.env.net_electric_consumption[-1] - self.current_demand))**3.0)
            
            
        #print(self.current_demand)
        #print(self.env.net_electric_consumption[-1])

        
        #rewards = (1-self.ramping_penalty )*(rewards) - self.ramping_penalty *((np.abs(self.env.net_electric_consumption[-1] - self.current_demand))**3.0)
        #rewards = (1-self.ramping_penalty )*(rewards) - self.ramping_penalty *((np.abs(self.env.buildings.values().current_net_electricity_demand - self.current_demand))**3.0)
        
        self.current_demand = self.env.net_electric_consumption[-1]
        
        #print('for next step ',self.current_demand)
        #print(self.env.net_electric_consumption[-1])
            #print(rewards)
        #print(rewards.shape)
            #sys.exit()
