...
from multi_agent_sac_pca_norm import SAC_multi_agent
...
parser.add_argument('--n_episodes', type=int, default=30, metavar='N',
...
env = CityLearn(data_path, building_attributes, weather_file, solar_profile, building_id,
                ...
                central_agent=False, simulation_period=(0, 8760 - 1))
...
if args.log:
    parent_dir = "multi_agent/multi_sac_all_build_only_std_pca_hid{}_bs_{}_{}_rew3/".format(hid[0],bs,time.strftime("%Y%m%d-%H%M%S"))  # apprends the timedate
...
agents = SAC_multi_agent(env,building_id, building_state_actions, building_info, observation_spaces=observations_spaces,
                          ...
                          pca_compression = 1,action_scaling_coef=0.5, reward_scaling=5., update_per_step=1, seed=0, add_previous_consumption=False)
...
best_total_score = 1.2
best_ramping_score= 1.2
...
for e in range(args.n_episodes):
    ...
    rewards = []
    #episode_reward = 0
    episode_reward = [0] * len(building_id)
    ...
    while not done:
        ...
        if agents.time_step >= agents.start_training and agents.batch_size <= len(agents.replay_buffer[agents.building_ids[0]]):
    ...
    if env.cost()['total'] < 0.98 and env.cost()['ramping'] < best_ramping_score :
        #best_total_score = env.cost()['total']
        best_ramping_score = env.cost()['ramping']
    ...
    print('Episode',e,'Loss -', env.cost(), 'Simulation time (min) -','episode_reward',sum(episode_reward), (time.time() - start) / 60.0, 'episode time(min) -',
