...
class SAC_single_agent:
    def __init__(self, building_ids, buildings_states_actions, building_info, observation_spaces=None,
                 ...
                 action_scaling_coef=1., reward_scaling=1., update_per_step=1,seed=0,add_previous_consumption=False):
        ...
        if self.add_previous_consumption:
            state_dim = len(self.observation_spaces.low) + 1
        else:
    ...
    def add_to_buffer(self, states, actions, rewards, next_states, done):
        ...
        if self.time_step >= self.start_training and self.batch_size <= len(self.replay_buffer):
            ...
            if self.norm_flag == 0:
                ...
                for s, a, r, s2, dones in self.replay_buffer.buffer:
                    s_buffer = np.clip((s - self.norm_mean)/self.norm_std,-1,1)
                    s2_buffer = np.clip((s2 - self.norm_mean)/self.norm_std,-1,1)
            ...
            if self.norm_flag == 0:
                ...
                R = np.array([j[2] for j in self.replay_buffer.buffer])
                self.r_norm_min = np.min(R)
                self.r_norm_max = np.max(R)
                print(self.norm_min)
                ...
                for s, a, r, s2, dones in self.replay_buffer.buffer:
                    ...
                    s_buffer_num = (s - self.norm_min)*(2)
                    s_buffer_den = (self.norm_max-self.norm_min)
                    s_buffer_den[s_buffer_den==0]=1.0
                    s_buffer = self.norm_min + s_buffer_num/s_buffer_den
                    #print(s_buffer_num)
                    #print(s_buffer_den)
                    #print(s_buffer)
                    
                    s2_buffer_num = (s2 - self.norm_min)*(2)
                    s2_buffer = self.norm_min + s_buffer_num/s_buffer_den
                    
                    r_buffer_num = (r - self.r_norm_min)*(2)
                    r_buffer_den = (self.r_norm_max-self.r_norm_min)
                    r_buffer_den=r_buffer_den + (r_buffer_den is 0)
                    r_buffer = self.r_norm_min + r_buffer_num/r_buffer_den
                    #print(r_buffer)
                    
                    #sys.exit()
                    new_buffer.append((s_buffer, a, r_buffer, s2_buffer, dones))
        ...
        if self.norm_flag == 1 :
            ...
            """
            states_num = (states - self.norm_min)*(2)
            states_den = (self.norm_max-self.norm_min)
            states_den[states_den==0]=1.0
            states = self.norm_min + s_buffer_num/states_den

            next_states_num = (next_states - self.norm_min)*(2)
            next_states = self.norm_min + next_states_num/s_buffer_den

            rewards_num = (rewards - self.r_norm_min)*(2)
            rewards_den = (self.r_norm_max-self.r_norm_min)
            rewards_den = rewards_den + (rewards_den)
            rewards = self.r_norm_min + rewards_num/rewards_den
            """
